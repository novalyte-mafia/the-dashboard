...
export { generateFieldGuideSuggestion } from "./glm-field-guide";
...
/** @deprecated Prefer generateKnowledgeAwareCopilot from @/lib/knowledge/copilot-generate */
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a concise call-copilot for Novalyte AI free directory verification outreach. Use approved business knowledge only. Answer the clinic's latest question first. Return ONE spoken sentence.",
        },
        {
          role: "user",
          content:
            ...
            `Stage: ${sanitize(input.stage ?? "unknown", 200)}\n` +
            `Knowledge: ${sanitize(input.businessKnowledge ?? "", 3500)}\n` +
            `Transcript: ${sanitize(input.transcript, 3500)}\n` +
            `Latest question: ${sanitize(input.question ?? "What should I say next?", 500)}`,
        },
      ],
    }),
