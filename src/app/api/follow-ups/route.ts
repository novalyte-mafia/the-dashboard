...
export async function POST(req: NextRequest) {
  ...
  const task = await db.followUpTask.create({
    data: {
      ...
      ...(body.relatedCallId ? { relatedCallId: String(body.relatedCallId) } : {}),
      ...(body.relatedDealId ? { relatedDealId: String(body.relatedDealId) } : {}),
      taskType: String(body.taskType ?? "general_task"),
      priority: String(body.priority ?? "normal"),
      dueDate: body.dueDate ? new Date(body.dueDate as string) : null,
      description: (body.description as string) ?? (body.notes as string) ?? null,
